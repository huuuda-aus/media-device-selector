...
export function checkEnvironmentSupport(): EnvironmentSupport {
  ...
  const isGetUserMediaSupported = isMediaDevicesSupported && 'getUserMedia' in navigator.mediaDevices;
  const isEnumerateDevicesSupported = isMediaDevicesSupported && 'enumerateDevices' in navigator.mediaDevices;
